		...
		<div class="my-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid gap-7 sm:grid-cols-2 lg:grid-cols-4">
                <div class="p-5 bg-white rounded shadow-sm">
                    <div class="text-base text-gray-400 ">Total Enumerators</div>
                    <div class="flex items-center pt-1">
                        <div class="text-2xl font-bold text-gray-900 ">{{ $enumerators->count() }}</div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('xlsx.export-selected-farmers') }}" method="POST" id="export-farms-from-selected-farmers">
                    @csrf
                            ...
                            <tbody>
                                @isset($enumerators)
                                    @if ($enumerators->count() > 0)
                                        ...
                                        <tr>
                                            <td class="text-xs"><input onchange="selectFarmer(event)" type="checkbox" name="selectedFarmers[]" id="checkbox-export-enumerator" class="checkbox-farmers" value="{{ $enumerator->id }}"></td>
                                            <td class="text-xs">{{ empty($enumerator->firstName) ? ' - ' : $enumerator->firstName }}</td>
                                            <td class="text-xs">{{ empty($enumerator->lastName) ? ' - ' : $enumerator->lastName }}</td>
                                            <td class="text-xs">{{ empty($enumerator->age) ? ' - ' : $enumerator->age }}</td>
                                            <td class="text-xs">{{ empty($enumerator->dateOfBirth) ? ' - ' : $enumerator->dateOfBirth }}</td>
                                            <td class="text-xs">{{ empty($enumerator->streetAddress) ? ' - ' : Str::substr($enumerator->streetAddress, 0, 20)."..." }}</td>
                                            <td class="text-xs">{{ empty($enumerator->barangay) ? ' - ' : $enumerator->barangay }}</td>
                                            <td class="text-xs">{{ empty($enumerator->city) ? ' - ' : $enumerator->city }}</td>
                                            <td class="text-xs">{{ empty($enumerator->municipality) ? ' - ' : $enumerator->municipality }}</td>
                                            <td class="text-xs">{{ empty($enumerator->phoneNumber) ? ' - ' : $enumerator->phoneNumber }}</td>
                                            <td class="text-xs">
                                                <!-- Using utilities: -->
                                                {{-- <div class="flex">
